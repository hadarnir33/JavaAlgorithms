...
/**
 * This class provides methods for working with strings and matrices.
 */
public class Ex13 {

    /**
     * Calculates the shortest common super-sequence of two strings using recursion.
    ...
    public static String minimalSt(String st1, String st2) {
        ...
        if (st1.charAt(0) == st2.charAt(0)) {
            ...
            // Returns the shorter string between the 2 options
            return option1.length() > option2.length() ? option2 : option1;
        }
    }

    /**
     * Checks if given indexes are at the border of a matrix.
     *
     * @param mat The matrix.
     * @param i   The x (axis) index.
     * @param j   The y (axis) index.
     * @return boolean Returns true if the indexes are at the border, false otherwise.
     */
    ...
    /**
     * Calculates and explores possible snake paths in a matrix and returns
     ...
     * 3. The next cell can only be one from above, below, right, left. not diagonal
     * 4. The next cell can only be a cell where the difference of the values is max 1
     * 5. The path cannot return to the previous cell
     *
     * @param mat     The matrix.
     * @param i       The x (axis) index.
     * @param j       The y (axis) index.
    ...
    public static int maxSnake(int[][] mat, int i, int j, int prevVal, boolean[][] visited) {
        ...
        // Base case - the indexes are at the border
        if (isBorder(mat, i, j))
            return 0;

        // Base case - the difference between the cells' values is greater than 1 or the cell is visited
        ...
        // Mark the current cell as visited to avoid looping back and forth between the same cells
        visited[i][j] = true;
    ...
    /**
     * Calculates the maximum length of a "snake-path" in a given 2D matrix.
     ...
     * @return int The length of the longest snake-path. If no valid path is found,
