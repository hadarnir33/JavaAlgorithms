...
public class IntListTwo {
    ...
    /**
     * Constructs an empty IntListTwo.
     */
    public IntListTwo() {
        _head = null;
        _tail = null;
     ...
     * If the list is empty, returns "{}".
     *
     * @return A string representing the list in reverse order.
     */
     ...
     * @return True if a valid path is found from the current node to the tail, false otherwise.
     */
    private boolean isWay(IntNodeTwo currentNode, IntNodeTwo leftBoundaryNode,int steps, boolean goRight) {
        // Base case: There is a way in the list
        ...
        // Base case: Reached tail, there is a way in the list
        if (currentNode == _tail)
            return true;

        // Base case: The current node reached the left boundary - meaning that the current node returned to a
        // visited node
        ...
        // Explore the left and right "ways" of the node
        return isWay(currentNode, leftBoundaryNode, currentNode.getNum(), true) ||
                isWay(currentNode, leftBoundaryNode, currentNode.getNum(), false);
    }

    /**
     * Determines whether there is a valid path through the list starting from the head.
     ...
     * tail of the list.
     *
     * @return True if a valid path exists from head to tail, false otherwise.
     */
